...
- Mono variants at both subheading sizes for IDs, codes, and tabular values, so numeric content stays aligned without ad-hoc overrides.
- Two token tiers, mirroring color: primitives hold the raw font, size, weight, and line-height values; semantic text styles bind them into named roles, so a size or leading change updates everywhere it's used.
...
- Two token tiers: primitives define the raw palette; semantics assign each value a role, so nothing in the product references a hex value directly.
- Every primitive holds a light and a dark value, so theming is a mode switch, not a second palette to maintain.
