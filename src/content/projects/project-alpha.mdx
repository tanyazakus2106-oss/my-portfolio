...
- **Palette:** 11 base colors with up to 8 shades each, plus a dedicated 10-hue data-visualization palette — a significant expansion from the previous system.
- **Accessibility:** A base shade defined for every color to ensure WCAG compliance, verified for both small text and graphics.
- **Applications:** A range of hues and shades covering every use — text, surfaces, borders, states, illustrations, and data visualization.

#### Elevation

- **Elevation scale:** A 5-level scale built from soft, layered shadows for realistic depth without harsh, artificial edges.
- **Directional & dark mode:** Left, right, and top shadow variants, each with light and dark mode support, so elevation adapts to context and surface.
- **Guidelines:** Hierarchy guidance per level, including shadow usage on colored backgrounds and the shift between flat and neumorphic styling.
